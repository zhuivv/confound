...
# estimation function
subsetfunction <- function(X_LDL_subset,X_demo = X_demo,Y_FA = Y_FA,B = 100,rate = 0.9){
  # Y_FA <- get("Y_FA", envir = .GlobalEnv)
  # X_demo <- get(demo, envir = .GlobalEnv)
  Y_FA_res = apply(Y_FA,2,function(y){
    fit <- lm(y~X_demo)
    residuals(fit)
  })
  X_LDL_res = apply(X_LDL_subset,2,function(y){
    fit <- lm(y~X_demo)
    residuals(fit)
  })
  greed1 = greedy_l0(Y_FA_res,ncol(X_LDL_res),X_LDL_res,rate =rate)
  estS1 = greed1$truest
  parallest1 <- debias_twostage_data(X_LDL_res, Y_FA_res, estS1)
  deconfoundest1 <- debias_deconfound_data(X_LDL_res, Y_FA_res, estS1)
  
  # bootstrap interval
  process_function_boot_res <- function(b) {
    # parameters
    X = X_LDL_res; Y = Y_FA_res; ptrue = estS1
  ...
  clusterExport(cl, c("debias_twostage_data", "debias_deconfound_data", 
                      "X_LDL_res","Y_FA_res","estS1","process_function_boot_res",
